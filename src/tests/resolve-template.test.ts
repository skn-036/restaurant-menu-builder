...
describe('Generate html string from template', () => {
    describe('mount restaurant information', () => {
        const templateInformationWithoutData = {
            ...templateInformation,
            restaurant_information: {},
            products: [],
        };
        const templateString = resolveTemplateString(
            templateInformation as TemplateInformation
        );

        const templateStringWithoutData = resolveTemplateString(
            // @ts-ignore
            templateInformationWithoutData
        );
        it('should mount restaurant and product template', () => {
            expect(
                templateString.includes('[{restaurant_template}]')
            ).toBeFalsy();
            expect(templateString.includes('[{product_template}]')).toBeFalsy();
        });

        it('should mount restaurant name if restaurant information is given', () => {
            expect(
                templateString.includes('test_restaurant_name')
            ).toBeTruthy();
        });

        it('should not mount restaurant name if restaurant information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_restaurant_name')
            ).toBeFalsy();
        });

        it('should mount restaurant description if restaurant information is given', () => {
            expect(
                templateString.includes('test_restaurant_description')
            ).toBeTruthy();
        });

        it('should not mount restaurant description if restaurant information is not given', () => {
            expect(
                templateStringWithoutData.includes(
                    'test_restaurant_description'
                )
            ).toBeFalsy();
        });

        it('should mount restaurant logo if restaurant information is given', () => {
            expect(
                templateString.includes('test_restaurant_logo')
            ).toBeTruthy();
        });

        it('should show restaurant logo if restaurant information is given', () => {
            expect(templateString.includes('display: block;')).toBeTruthy();
        });

        it('should not mount restaurant logo if restaurant information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_restaurant_logo')
            ).toBeFalsy();
        });

        it('should not show restaurant logo if restaurant information is not given', () => {
            expect(
                templateStringWithoutData.includes('display: none;')
            ).toBeTruthy();
        });
    });

    describe('mount product information', () => {
        const templateInformationWithoutData = {
            ...templateInformation,
            restaurant_information: {},
            products: [],
        ...
        it('should mount product name if product information is given', () => {
            expect(templateString.includes('test_product_name')).toBeTruthy();
        });

        it('should not mount product name if product information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_product_name')
            ).toBeFalsy();
        });

        it('should mount product description if product information is given', () => {
            expect(
                templateString.includes('test_product_description')
            ).toBeTruthy();
        });

        it('should not mount product description if product information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_product_description')
            ).toBeFalsy();
        });

        it('should mount product price if product information is given', () => {
            expect(templateString.includes('test_product_price')).toBeTruthy();
        });

        it('should not mount product price if product information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_product_price')
            ).toBeFalsy();
        });

        it('should mount product logo if product information is given', () => {
            expect(templateString.includes('test_product_logo')).toBeTruthy();
        });

        it('should show product logo if product information is given', () => {
            expect(templateString.includes('display: block;')).toBeTruthy();
        });

        it('should not mount product logo if product information is not given', () => {
            expect(
                templateStringWithoutData.includes('test_product_logo')
            ).toBeFalsy();
        });

        it('should not show product logo if product information is not given', () => {
            expect(
                templateStringWithoutData.includes('display: none;')
            ).toBeTruthy();
        });
